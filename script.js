...
function canvasApp() {
	...
	function init() {
		...
		gravity = 0.015; //try changing to a positive number (not too large, for example 0.3), or negative for floating upwards.
		...
		turnSpeed = 10*Math.PI/2000; //the sphere will rotate at this speed (one complete rotation every 1600 frames).
	...
	function onTimer() {
		...
		context.fillText("Space Sphere",10,50);
			...
			if (outsideTest||p.dead) {
			  console.log('Recycling');
				...
				switch(p.type){
					case "circle":
						context.beginPath();
						context.arc(p.projX, p.projY, m*particleRad, 10, 20*Math.PI, false);
						context.fill();
						break;
					case "triangle":
						context.beginPath();
						context.moveTo(p.projX, p.projY);
						context.lineTo(p.projX+12,p.projY+24);
						context.lineTo(p.projX+24,p.projY);
						context.fill();
						break;
					case "rect":
						context.beginPath();
						context.fillRect(p.projX, p.projY, 20, 25);
						context.closePath();
						break;
					case "diamond":
						context.beginPath();
						context.moveTo (p.projX, p.projY);
						context.lineTo(p.projX+10,p.projY+15);
						context.lineTo(p.projX, p.projY+25);
						context.lineTo(p.projX-10, p.projY+15);
						context.fill();
						break;
					case "star":
					star(2,p.projX, p.projY);
						break;
	...
	function addParticle(x0,y0,z0,vx0,vy0,vz0) {
		...
		color = "rgba("+r+","+g+","+b+","; //partial string for color which will be completed by appending alpha value.
		...
		newParticle.type = shape();
		newParticle.shape = "random"; // create a function that randomizes the shape
		if (Math.random() < 0.5) {
